...
export default function BuscarContactos() {
  ...
  useEffect(() => {
    ...
    const s = ctrl.signal
    api.get(API_APOLLO_STATUS, { signal: s }).then(({ data }) => setApolloOk(!!data?.data?.configurado)).catch(() => { if (!s.aborted) setApolloOk(false) })
    api.get(API_PERPLEXITY_STATUS, { signal: s }).then(({ data }) => setPerplexityOk(!!data?.data?.configurado)).catch(() => { if (!s.aborted) setPerplexityOk(false) })
    api.get(API_APIFY_STATUS, { signal: s }).then(({ data }) => setApifyOk(!!data?.data?.configurado)).catch(() => { if (!s.aborted) setApifyOk(false) })
    return () => ctrl.abort()
  }, [])
  ...
  const COLUMNS = [
    ...
    { key: 'nombre', label: 'Nombre' }, { key: 'empresa', label: 'Empresa' }, { key: 'cargo', label: 'Cargo' },
    { key: 'email_empresarial', label: 'Email Corp.' }, { key: 'email_personal', label: 'Email Personal' },
    { key: 'telefono_empresa', label: 'Tel. Empresa' }, { key: 'telefono_personal', label: 'Tel. Personal' },
    ...
    { key: '_estado', label: 'Estado', width: '130px', render: (_, row) => row.ya_existe ? (
        <div className="flex gap-sm" style={{ alignItems: 'center' }}>
          <span style={{ fontSize: '0.72rem', fontWeight: 600, padding: '2px 7px', borderRadius: 10, background: '#D1FAE5', color: '#065F46', whiteSpace: 'nowrap' }}>Ya en KarIA</span>
          <button title="Enriquecer contacto existente" disabled={enrichingId === row.contact_id_existente}
            onClick={(e) => { e.stopPropagation(); enriquecerExistente(row) }}
            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1rem', opacity: enrichingId === row.contact_id_existente ? 0.4 : 1 }}>
            {enrichingId === row.contact_id_existente ? '...' : '⚡'}</button>
        </div>
      ) : <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>Nuevo</span>,
    },
  ]

  const buscarConAPI = async (endpoint, payload) => {
    setLoading(true)
    try {
  ...
  const handleApolloBuscar = async (apolloParams) => {
    if (!apolloOk) return toast.error('Configurá tu API key de Apollo en Configuración')
    if (!form.rubro.trim() && !form.prompt_personalizado?.trim()) return toast.error('Ingresa un rubro o un prompt personalizado')
    const payload = { ...form, ...apolloParams }
    if (!payload.prompt_personalizado?.trim()) delete payload.prompt_personalizado
    await buscarConAPI(API_APOLLO_SEARCH, payload)
  }
  ...
  const enriquecerExistente = async (row) => {
    ...
    setEnrichingId(cid)
    try { await api.post(API_CONTACT_ENRICH(cid), { metodo }); toast.success('Contacto enriquecido con los nuevos datos') }
    catch (err) { toast.error(err.message) }
  ...
  const toggleSelect = (row) => {
    ...
    setResults((prev) => prev.map((c) => { const k = c.id || c.email_empresarial || c.email_personal; return k === key ? { ...c, _selected: !c._selected } : c }))
  }
  ...
  const guardar = async () => {
    ...
    try { const { data } = await api.post(API_CONTACTS_SAVE, { contactos: sel }); toast.success(`${data.guardados} contactos guardados`); setResults([]) }
    catch (err) { toast.error(err.message) }
  }
  ...
  const armarBloque = async () => {
    ...
    try {
      ...
      if (ids.length) { const { data: bd } = await api.post(API_BLOQUES, { nombre: nombreBloque.trim() }); await api.post(API_BLOQUE_CONTACTOS(bd.data.id), { contacto_ids: ids }) }
  ...
  const agregarManual = async () => {
    try { await api.post(API_CONTACTS_MANUAL, manual); toast.success('Contacto agregado'); setShowManual(false); setManual({ nombre: '', empresa: '', email_empresarial: '', cargo: '' }) }
    catch (err) { toast.error(err.message) }
  }
  ...
  return (
    <div>
      <div className="card mb-md">
        ...
        {metodo === 'perplexity' && perplexityOk === false && <p className="text-sm text-secondary" style={{ marginTop: '0.5rem' }}>Configurá tu API key de Perplexity en Configuración</p>}
        {metodo === 'apify' && apifyOk === false && <p className="text-sm text-secondary" style={{ marginTop: '0.5rem' }}>Configurá tu API key de Apify en Configuración</p>}
      </div>
      {results.length > 0 && (
        <div className="card">
      ...
      {showBloque && (
        <Modal title="Armar bloque" onClose={() => setShowBloque(false)}>
          <div className="form-group">
            ...
            <input id="bloque-nombre" value={nombreBloque} onChange={(e) => setNombreBloque(e.target.value)} placeholder="Ej: Hospitales Córdoba" />
          </div>
